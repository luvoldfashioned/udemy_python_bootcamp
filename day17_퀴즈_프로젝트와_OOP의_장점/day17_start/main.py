...
class User:
    ...
    def follow(self, user):   # 메소드는 함수와 다르게 항상 첫 매개변수로 self 매개변수가 있어야 함
        user.followers += 1
        self.following += 1   # self: 클래스 청사진 내부에서 클래스로부터 만들어지는 객체를 지칭
...
# Constructor(생성자): 청사진의 일부, 객체가 생성될 때 무슨 일이 일어나야 하는지 명시할 수 있게 함
