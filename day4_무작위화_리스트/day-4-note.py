...
random_integer = random.randint(1, 10)  #정수 난수 생성 random.randint(범위, 범위)
print(random_integer)
random_float = random.random() #0~1사이의 부동 소수점 난수 생성
print(random_float)
...
states_of_america = ["Delaware", "Pennsylavania"] #list는 순서가 있다
...
#함수를 암기할 필요 없다. 비효율적, 자료를 읽은 후 어떤 작업이 가능할 지 생각해보기. 프로그래밍은 오픈북 시험이다.
